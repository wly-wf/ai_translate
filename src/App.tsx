  ...
  function renderProviderDetails() {
    ...
    return <div className="provider-detail-content">
      ...
      <section className="model-section">
        <div className="model-section-header"><div className="model-section-title"><h2>模型</h2><span>{selectedDraft.model ? "1" : "0"}</span></div><div className="model-toolbar"><button type="button" aria-label="搜索模型" title="搜索模型"><SearchIcon className="model-search-icon" /></button><button type="button" aria-label="选择模型" title="选择模型">☑</button><button type="button" aria-label="添加模型" title="添加模型">＋</button><button type="button" aria-label="删除模型" title="删除模型">♜</button><button className="fetch-models" type="button" onClick={() => setNotice("模型列表已刷新。")}>↻ 获取</button></div></div>
  ...
  function renderProviderDetails() {
    ...
    return <div className="provider-detail-page">
      ...
      <section className="settings-form-card provider-config-card">
        ...
        <div className="settings-field-group"><label className="field-label" htmlFor="provider-api-path">API 路径</label><input id="provider-api-path" value={apiPath} readOnly spellCheck={false} /></div>
        <div className="model-section provider-model-section"><div className="model-section-header"><div className="model-section-title"><h3>模型</h3><span>{selectedDraft.model ? "1" : "0"}</span></div><div className="model-toolbar"><button type="button" aria-label="搜索模型" title="搜索模型"><SearchIcon className="model-search-icon" /></button><button type="button" aria-label="选择模型" title="选择模型">☑</button><button type="button" aria-label="添加模型" title="添加模型">＋</button><button className="fetch-models" type="button" onClick={() => setNotice("模型列表刷新功能暂未接入，当前使用手动填写的模型。")}>↻ 获取</button></div></div><div className="model-group"><div className="model-group-heading"><strong>{selectedSettingsProvider.vendor}</strong></div><div className="model-row"><ProviderIcon provider={selectedSettingsProvider} /><label className="model-input-label" htmlFor="provider-model"><span className="sr-only">模型名称</span><input id="provider-model" value={selectedDraft.model} onChange={(event) => updateSelectedDraft("model", event.target.value)} spellCheck={false} /></label><button type="button" className="model-remove-button" onClick={() => updateSelectedDraft("model", "")} aria-label="移除模型" title="移除模型">−</button></div></div></div>
      </section>
      <div className="provider-detail-footer"><div className="action-row settings-actions"><button className="primary" onClick={() => void saveProviderConfig()}>保存配置</button></div></div>
      {connectionState.providerId === selectedSettingsProvider.id && connectionState.status !== "idle" && <p className={`connection-result ${connectionState.status}`} role="status">{connectionState.message}</p>}
    </div>;
