...
import { ProviderIcon, Icon, AvailableModelsDialog, ApiKeyInput } from "./sharedUI";
...
export function AddProviderWindow() {
  ...
  return <main className="app-shell add-provider-window-shell"><section className="settings-add-provider-page" aria-labelledby="add-provider-title">
    <div className="add-provider-titlebar" onMouseDown={beginWindowDrag}>
    ...
    <div className="add-provider-content">
      <div className="add-provider-scroll">
        ...
        <div className="add-provider-form">
          ...
          <div className="add-provider-field"><label htmlFor="add-provider-api-key">API Key</label><ApiKeyInput id="add-provider-api-key" value={draft.apiKey} onChange={(value) => updateDraft("apiKey", value)} required /></div>
    ...
    {modelDialogOpen && <AvailableModelsDialog provider={dialogProvider} models={fetchedModels} selectedModels={uniqueModels(draft.models)} isLoading={fetchingModels} fetchError={modelFetchError} closeButtonRef={modelDialogCloseRef} restoreFocusRef={modelFetchButtonRef} onToggle={toggleFetchedModel} onClose={closeModelDialog} onRetry={() => void fetchModels()} />}
  </section></main>;
}
